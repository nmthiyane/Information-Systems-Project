...
namespace PoppelProject.PresentationLayer
{
    public partial class OrderForm : Form
    {
        ...
        public void FillCombo()
        {
            ...
            products = productController.FindByStatus(Product.productStatus.notExpired);
        ...
        public bool CheckAvailability() // check if the item is available
        {
            ...
            {
                MessageBox.Show("Sorry, it seems as if " + product.ProductName + "is out of stock");
                return false;
            }

            else if (qtyNumericUpDown.Value > product.QuantityInStock)
            {
                MessageBox.Show("Only " + product.QuantityInStock + " " + product.ProductName + " available in stock, please select items less or equal to " + product.QuantityInStock);
                return false;
            }
        ...
        {
            int noOfOrders = orderController.AllOrders.Count + 1000000;
            return "ORD" + noOfOrders;
        }

        public string generateOrderItemID()  // To generate a unique order item ID
        {
            int noOfOItems = orderItemsController.AllOrderItems.Count + 1000000;
            return "ITE" + noOfOItems;
        }
